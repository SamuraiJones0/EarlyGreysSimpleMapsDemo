<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.9" tiledversion="1.9.2" name="forest_ [resources]" tilewidth="16" tileheight="16" tilecount="120" columns="12">
 <image source="tilesets/fantasy_ [version 2.0]/forest_/forest_ [resources].png" width="192" height="160"/>
 <tile id="91">
  <animation>
   <frame tileid="91" duration="100"/>
   <frame tileid="92" duration="100"/>
   <frame tileid="93" duration="100"/>
   <frame tileid="94" duration="100"/>
  </animation>
 </tile>
 <tile id="103">
  <animation>
   <frame tileid="103" duration="100"/>
   <frame tileid="104" duration="100"/>
   <frame tileid="105" duration="100"/>
   <frame tileid="106" duration="100"/>
  </animation>
 </tile>
</tileset>
